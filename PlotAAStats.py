...
if __name__ == "__main__":
	...
	frames = []
	for domain in domains:
		path = os.path.join(input, os.path.join(domain, os.path.join("output", "code_clr_corr.csv")))
		df = pd.read_csv(path, sep="\t", header=0, index_col=0)
		df["Domain"] = [domain] * len(df)
		frames.append(df)
	...
	frames = []
	for domain in domains:
		path = os.path.join(input, os.path.join(domain, os.path.join("output", "gc_clr_corr.csv")))
		df = pd.read_csv(path, sep="\t", header=0, index_col=0)
		df["Domain"] = [domain] * len(df)
		frames.append(df)
