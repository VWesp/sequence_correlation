...
if __name__=="__main__":
    ...
    for calc in ["sum", "median"]:
        ...
        fig.suptitle(f"{name} - Neighboring {type} {calc} counts", fontsize=14,
                     y=0.93)

        for ext in ["svg", "pdf"]:
            plt.savefig(f"{type_output}/{type}_context_{calc}_counts.{ext}",
                        bbox_inches="tight")
        ...
        for step,df in enumerate(z_frames):
            ...
            df.to_csv(os.path.join(csv_output,
                                   f"{type}_context_{calc}_z_scores.csv"),
                      sep="\t")

            i = i+1 if i < 2 else 0
            j = j+1 if i == 0 else j

        fig.suptitle(f"{name} - Neighboring {type} {calc} Z-scores",
                     fontsize=14, y=0.93)

        for ext in ["svg", "pdf"]:
            plt.savefig(f"{type_output}/{type}_context_{calc}_z_scores.{ext}",
                        bbox_inches="tight")
        ...
        for step,df in enumerate(context_frames):
            ...
            pca_df.to_csv(os.path.join(csv_output,
                          f"{type}_context_{calc}_pcas.csv"),
                          sep="\t")

            i = i+1 if i < 2 else 0
            j = j+1 if i == 0 else j

        fig.suptitle(f"{name} - Neighboring {type} {calc} PCA", fontsize=14,
                     y=0.93)

        for ext in ["svg", "pdf"]:
            plt.savefig(f"{type_output}/{type}_context_{calc}_pcas.{ext}",
                        bbox_inches="tight")
        ...
        if(type == "amino"):
            ...
            fig.suptitle(f"{name} - Amino acids neighbor {calc} counts",
                         fontsize=14, y=0.93)

            for ext in ["svg", "pdf"]:
                plt.savefig(f"{type_output}/{type}_context_{calc}_comp.{ext}",
                            bbox_inches="tight")
